...
/obj/item/gun/projectile/modkit
	name = "laser tag gun"
	icon = 'icons/obj/guns/energy/bluetag.dmi'
	icon_state = "bluetag"
	item_state = "bluetag"
	desc = "A mock projectile weapon with removeable cosmetic parts that can be attached to a real gun."
	origin_tech = list(TECH_COMBAT = 1, TECH_MAGNET = 2)
	matter = list(MATERIAL_PLASTIC = 6)
	fire_sound = 'sound/weapons/Laser.ogg'
	bad_type = /obj/item/gun/projectile/modkit
	spawn_tags = SPAWN_TAG_TOY_WEAPON
	spawn_blacklisted = TRUE
	rarity_value = 30
	var/modtype
...
/obj/item/gun/projectile/modkit/attackby(var/obj/item/O, var/mob/user)
	if(istype(O, modtype))
		to_chat(user, "You set about applying [src]\'s parts onto the [O].")
		if(spawn_new)
			var/turf/T = get_turf(user)
