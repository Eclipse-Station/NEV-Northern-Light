...
/obj/item/oddity/broken_necklace/examine(user, distance)
	...
	if(my_area.bluespace_entropy >= 0 && my_area.bluespace_entropy <= my_area.bluespace_hazard_threshold * 0.3)
		to_chat(user, SPAN_NOTICE("This feels cold to the touch."))
	else if(my_area.bluespace_entropy >= my_area.bluespace_hazard_threshold * 0.7)
