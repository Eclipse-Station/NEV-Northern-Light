...
proc/infection_check(var/mob/living/carbon/M, var/vector = "Airborne")
	...
	var/protection = M.getarmor(null, ARMOR_BIO)	//gets the full body bio armour value, weighted by body part coverage.
	...
	switch(vector)
		if("Airborne")
			...
			if(I)
				score = max(score, round(0.06*I.armor.getRating(ARMOR_BIO)))
...
/proc/infection_spreading_check(var/mob/living/carbon/M, var/vector = "Airborne")
	...
	if (vector == "Airborne")
		var/obj/item/I = M.wear_mask
		if (istype(I))
			protection = max(protection, I.armor.getRating(ARMOR_BIO))
