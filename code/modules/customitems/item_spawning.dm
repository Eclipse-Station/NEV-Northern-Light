...
/datum/custom_item/proc/apply_inherit_inhands(var/obj/item/item)
	...
	if(!("[item_icon]_l" in available_states))
		...
		new_item_icons[slot_l_hand_str] = get_icon(item, slot_l_hand_str, 'zzz_modular_eclipse/icons/mob/items/lefthand.dmi')
	if(!("[item_icon]_r" in available_states))
		new_item_state_slots[slot_r_hand_str] = get_state(item, slot_r_hand_str, "_r")
		new_item_icons[slot_r_hand_str] = get_icon(item, slot_r_hand_str, 'zzz_modular_eclipse/icons/mob/items/righthand.dmi')
